...
public class TowerInfoPanel_UI : MonoBehaviour
{
    ...
    public void UpdatePanelInfo(GameObject image, string name, List<string> stats, int price, List<string> description)
    {
        if(firstUpdate) 
        {
        ...
        {
            _infoTexts[i].text = _infoTexts[i].text.Split(':')[0] + ": " + stats[i];
        }
